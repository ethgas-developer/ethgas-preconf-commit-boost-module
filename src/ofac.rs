    ...
    for (i, chunk) in pubkeys.chunks(100).enumerate() {
        ...
        match res.json::<APIValidatorUpdateOfacResponse>().await {
            Ok(res_json) => {
                if res_json.success {
                    if enable_ofac {
                        info!("batch {}: successfully enabled ofac for the above registered validators", i + 1);
                    } else {
                        info!("batch {}: successfully disabled ofac for the above registered validators", i + 1);
                    }
                } else if enable_ofac {
                    error!(
