...
- Set ETHGas Commit module config in `config.toml`
  ...
  - `payout_address` is your actual fee recipient address for receiving payout if ETHGas collateral contract (EthgasPool) address is set as the fee recipient in your beacon node. More details can be found in [here](https://docs.ethgas.com/get-started/node-operators/multi-relay-support)
