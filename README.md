...
* Set ETHGas Commit module config in `config.toml`
    ...
    * for SSV validators, set keystore configurations under `ssv_node_operator_owner_keystores` array where each entry contains both `keystore_path` and `password_path`, and specify validator public keys under `ssv_node_operator_owner_validator_pubkeys` or set it as `[]` to indicate the registration of all associated validator public keys obtained via SSV official API
        ...
        * if node operators within a cluster are associated with different owner addresses which are all owned by you, please put all keystores of those owner addresses under the `ssv_node_operator_owner_keystores` array. This can ensure our exchange will open markets for your ssv validators even when the leading node operator rotates within the cluster.
